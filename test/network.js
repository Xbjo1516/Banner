...
async function analyzeUrl(url) {
  ...
  try {
    const titleEl = await page
      .locator("h1.campaign-heading.text-center")
      .first();
...
// ⭐ ฟังก์ชันที่ให้ Inquirer เรียกใช้
export async function run(bannerUrls = []) {
  let category = "Manual Input links";

  // ถ้าไม่ได้ส่ง URL มา → fallback ไปใช้ link.json
    ...
    for (const [domain, types] of Object.entries(domainData)) {
      ...
      for (const [type, files] of Object.entries(types)) {
        ...
        for (const [name, info] of Object.entries(files)) {
          lines.push(
            `    - ${name}: count = ${info.count}, size = ${formatSize(
              info.size
            )}`
          );
        }
