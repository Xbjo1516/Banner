...
async function analyzeBanner(browser, url, saveDir) {
  ...
  page.on("response", async (res) => {
    try {
      ...
      if (!frameData.has(frame))
        frameData.set(frame, { assets: [], totalKB: 0, totalCount: 0 });
  ...
  // reset initial load counter แล้วโหลดอีกรอบสำหรับการวัดจริง
  initialKB = 0;
  initialCount = 0;
  ...
  for (const [frame, data] of frameData.entries()) {
    const nameCount = {};
    data.assets.forEach(
      (a) => (nameCount[a.filename] = (nameCount[a.filename] || 0) + 1)
    );
    const duplicates = Object.entries(nameCount)
      .filter(
        ([name, count]) =>
          count > 1 && !/^track-video/i.test(name) // ตัดไฟล์ track-video ออก
      )
      .map(([name]) => name);
  ...
  return {
    bannerUrl: url,
    initial: { kb: initialKB.toFixed(2), count: initialCount },
    frames: frameReport,
  };
}

// ⭐ ฟังก์ชันที่ Inquirer จะเรียก
export async function run(bannerUrls = []) {
  const browser = await firefox.launch({ headless: true });

  let category = "Manual Input links";

  // ถ้าไม่ได้ส่ง URL มาจาก Inquirer → ใช้จาก link.json เหมือนเดิม
  ...
  for (const url of bannerUrls) {
    ...
    report.frames.forEach((f, i) => {
      ...
      lines.push(
        `Duplicates: ${
          f.duplicates.length ? f.duplicates.join(", ") : "None"
        }`
      );
      lines.push("");
    });
